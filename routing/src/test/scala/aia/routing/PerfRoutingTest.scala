...
import akka.actor.{ActorSystem, DeadLetter, PoisonPill, Props, SupervisorStrategy}
...
class PerfRoutingTest
    extends TestKit(ActorSystem("PerRoutingTest"))
    with MustMatchers
    with WordSpecLike
  ...
  "survive killed actor ref routee" in {
    ...
    router ! msg
    endProbe.expectMsgType[PerformanceRoutingMessage]
